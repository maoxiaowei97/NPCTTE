...
class DiffusionProcess:
    ...
    def p_losses(self, denoise_model, x_start, t, y=None, batch_traffic_state=None,  noise=None, loss_type="l1"):
        """
        ...
        predicted_noise = denoise_model(x_noisy, t, y, batch_traffic_state)
    ...
    @torch.no_grad()
    def p_sample(self, model, x, t, t_index, y=None, traffic = None):
        """
        ...
        model_mean = sqrt_recip_alphas_t * (
                x - betas_t * model(x, t, y, traffic) / sqrt_one_minus_alphas_cumprod_t
        )
    ...
    @torch.no_grad()
    def p_sample_loop(self, model, shape, y=None, traffic=None, display=False):
        """
        ...
        for i in iter:
            img = self.p_sample(model, img, torch.full((b,), i, device=device, dtype=torch.long), i, y=y, traffic = traffic)
