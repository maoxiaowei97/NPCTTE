...
class ConvNextBlock(nn.Module):
    ...
    """
    def __init__(self, dim, dim_out, traffic_state=False, *, time_emb_dim=None, mult=2, norm=True):
        super().__init__()
        self.mlp = (
    ...
    def forward(self, x, time_emb=None, traffic=None):
        ...
        if exists(self.mlp) and exists(time_emb):
            ...
            if self.traffic_state:
                h = h + rearrange(condition, "b c -> b c 1 1") + rearrange(traffic, "b c -> b c 1 1") # h + condition, condition可以变成odt + traffic_state(b, c, n, n). [b, 4, 20, 20], [b, 4, 1, 1]
            else:
    ...
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        block_klass(dim_in, dim_out, time_emb_dim=time_dim, traffic_state=self.traffic_state),
                        block_klass(dim_out, dim_out, time_emb_dim=time_dim, traffic_state=self.traffic_state),
        ...
        mid_dim = dims[-1]
        self.mid_block1 = block_klass(mid_dim, mid_dim, time_emb_dim=time_dim, traffic_state=self.traffic_state)
        self.mid_attn = Residual(PreNorm(mid_dim, Attention(mid_dim)))
        self.mid_block2 = block_klass(mid_dim, mid_dim, time_emb_dim=time_dim, traffic_state=self.traffic_state)
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            ...
            self.ups.append(
                nn.ModuleList(
                    [
                        block_klass(dim_out * 2, dim_in, time_emb_dim=time_dim, traffic_state=self.traffic_state),
                        block_klass(dim_in, dim_in, time_emb_dim=time_dim, traffic_state=self.traffic_state),
        ...
        self.final_conv = nn.Sequential(
            block_klass(dim, dim, traffic_state=self.traffic_state), nn.Conv2d(dim, out_dim, 1)
        )

        self.name = 'unet'
        self.num_layers = len(dim_mults)
    #input: X_noise, t, odt
    def forward(self, x, time=None, y=None, traffic=None): # distorted image, odt
        ...
        for block1, block2, attn, downsample in self.downs:
            x = block1(x, t, traffic)# x: [b, 4, h, w]
            x = block2(x, t, traffic)
            x = attn(x)
            h.append(x)
            x = downsample(x)

        # bottleneck
        x = self.mid_block1(x, t, traffic)
        x = self.mid_attn(x)
        x = self.mid_block2(x, t, traffic)

        # upsample
        for block1, block2, attn, upsample in self.ups:
            x = torch.cat((x, h.pop()), dim=1)
            x = block1(x, t, traffic)
            x = block2(x, t, traffic)
            x = attn(x)
            x = upsample(x)
