...
class TransformerPredictor(nn.Module):
    ...
    def forward(self, x, odt):
        """
        :param x: input trajectory images, shape (N, k, num_channel, dim, dim). [mean, nppc]
        :param odt:
        :return:
        """
        # if torch.isnan(x_).any():
        #     print('input contains nan !')
        # if self.predict_type == 'only_mean':
        #     # x = torch.concat([x_[:, 0], x_[:, 0]], dim=0).clone()
        #     x = x_[:, 0].clone()
        # elif self.predict_type == 'only_nppc':
        #     # x[:, 0] = x[:, 1]
        #     # x = torch.concat([x_[:, 1], x_[:, 1]], dim=0).clone()
        #     x = x_[:, 1].clone()
        # elif self.predict_type == 'mean_nppc':
        #     x = x_
        # else:
        #     x =  x_
        shape = x.shape
        # x = x.reshape(x_.shape[0] * x_.shape[1], x_.shape[2], x_.shape[3], x_.shape[4]) # [b, k, c, n, n] -> [b*k, c, n, n]
        ...
        # out = self.transformer(pos)  # (N, num_grid, d_model)
        # out = pos
